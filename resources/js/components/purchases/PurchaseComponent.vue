        ...
        <div class="grid w-full grid-flow-row px-4 mx-auto md:grid-cols-5">
            <div class="
                ...
                <div class="mt-2 md:mt-0">
                    <complete-purchase-btn :purchase="purchase" :total-purchase="localTotalPurchase" />
            ...
            <div class="w-full col-span-5">
                <errors-component :errors-found="errorsFound"></errors-component>
            </div>

            <div v-if="isAdmin && !user.inventory_id" class="
                    col-span-5
                    px-4
                    py-2
                ...
                <div class="grid grid-cols-1 md:grid-cols-5">
                    ...
                    <label v-for="inventory in inventories" class="
                            flex flex-wrap
                            items-center
                            justify-center
                            p-2
                        ...
                        <input type="radio" name="inventory_id" class="form-radio"
                            @change="selectedInventory(inventory)" :value="inventory.id"
                            :checked="inventory.id == purchase.inventory_id" />
                    </label>
                </div>
            </div>

            <product-list v-if="localPurchase.status === 'pending'" :products="localProductsInPurchase"
...
export default {
    ...
    mounted() {
        ...
        EventBus.$on("purchase-completed", this.purchaseCompleted);
    },
    beforeUnmount() {
        EventBus.$off("purchase-extracted", this.removeProductFromPurchase);
        EventBus.$off("total-updated-purchase", this.changeTotal);
        EventBus.$off("purchase-completed", this.purchaseCompleted);
    },
